...
def get_ticket(host):
    ...
    display_action_name("1. Client calls GG Proxy API without RPT token")
...
def get_rpt(access_token, ticket):
    ...
    response = requests.post(("%s/uma-rp-get-rpt" % oxd_host),
                             ...
                             json=body,
                             verify=False)
    display_action_name("2. Client calls AS UMA /token endpoint with permission ticket and client credentials")
    display_response(response, body)

    if 'error' in response.json() and response.json()['error'] == "need_info":
        return True, "", response.json()['details']['redirect_user']
    else:
...
def call_gg_rpt(host, rpt):
    ...
    display_action_name("3. Client calls GG Proxy API with RPT token.")
    display_response(response, "")
